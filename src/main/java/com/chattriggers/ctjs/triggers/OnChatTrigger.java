...
public class OnChatTrigger extends OnTrigger {
    ...
    /**
     * Argument 1 (ServerChatEvent) the chat event fired
     * @param args list of arguments as described
     */
    @Override
    public void trigger(Object... args) {
        if (!(args[0] instanceof ServerChatEvent))
            throw new IllegalArgumentException("Argument 2 must be a ServerChatEvent");

        ServerChatEvent chatEvent = (ServerChatEvent) args[0];

        if (!this.triggerIfCanceled && chatEvent.isCancelled()) return;

        String chatMessage = getChatMessage(chatEvent);
    ...
    private String getChatMessage(ServerChatEvent chatEvent) {
        String chatMessage = chatEvent.getChat().getUnformattedText();

        if (this.chatCriteria.contains("&"))
            chatMessage =  chatEvent.getChat().getFormattedText().replace("\u00a7", "&");
